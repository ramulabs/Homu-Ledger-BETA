---
id: health-security-51f6038af2
title: updateRecurringItem updates by id with no household-scoping
status: backlog
priority: P2
assignee: unassigned
...
labels:
  ...
  - Warning
  - Security
created_at: 2026-06-15T19:17:59.811Z
updated_at: 2026-06-15T19:17:59.811Z
---
...
`updateRecurringItem` operates on the `recurring_items` table filtering only by `id` — no `.eq("household_id", householdId)` scope is applied. While Supabase RLS may enforce household-level isolation at the DB layer, the server action itself provides no defense-in-depth ownership check.
