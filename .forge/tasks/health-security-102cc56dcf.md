...
labels:
  ...
  - warning
  - health-check
created_at: 2026-06-02T19:11:21.837Z
updated_at: 2026-06-02T19:11:21.837Z
---
...
Contrast with `declineInvitation`, which correctly validates `invite.invited_user_id !== user.id`. Any authenticated user who learns or guesses a valid `invitationId` UUID can cancel any pending invitation in any household — depending entirely on whether the Supabase RLS `DELETE` policy enforces ownership.
...
Also audit the RLS `DELETE` policy on `household_invitations`.
